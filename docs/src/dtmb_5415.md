...
Twelve compatible F-Spline sections, five longitudinal form curves, and their
...
The generated hull is one tensor-product B-spline surface. The two physical
dome-transition locations are repeated internal knots with $C^1$ continuity,
not patch interfaces. Orange, gold, and blue identify curve-network intervals
inside that one surface. Gray solid curves are canonical IGES evaluations and
colored dashed curves are generated-surface evaluations. No opaque surface or
...
| Fitting-section RMS | $9.282\ \mathrm{mm}$ |
| Fitting-section maximum | $49.546\ \mathrm{mm}$ |
| Averaged-knot single-patch independent RMS | $16.164\ \mathrm{mm}$ |
| Feature-aligned single-patch independent RMS | $15.063\ \mathrm{mm}$ |
| Feature-aligned single-patch independent maximum | $60.609\ \mathrm{mm}$ |

The diagnostic plot compares two single surfaces built from the same solved
sections. Feature-aligned knots reduce independent RMS by $6.81\%$ relative to
standard averaged knots. The sonar template exactly controls dome depth,
maximum breadth and height, attachment breadth and height, and the tangent at
maximum breadth. Its larger maximum holdout error is an important negative
result: fitting those quantities independently at only two sections does not
yet define their longitudinal evolution. The next refinement is therefore a
set of fair sonar-dome auxiliary curves along the hull, connected to the
adaptive-stem curve, rather than more isolated hard section constraints.
