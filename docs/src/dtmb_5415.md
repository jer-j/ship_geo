...
Ten compatible F-Spline sections, five longitudinal form curves, and their KKT
...
dashed curves are evaluations of the generated tensor-product surface. No
opaque surface patch or control net is drawn over the comparison.
...
| Fitting-section RMS | $9.826\ \mathrm{mm}$ |
| Fitting-section maximum | $39.470\ \mathrm{mm}$ |
| Independent-section RMS | $15.163\ \mathrm{mm}$ |
| Independent-section maximum | $53.414\ \mathrm{mm}$ |

The diagnostic plot shows how the auxiliary form functions balance local data
against global fairness and exact primary constraints. The largest independent
section errors occur in the sonar-dome/bow and stern regions. This is important:
the single longitudinal tensor-product patch is too restrictive where DTMB 5415
changes character rapidly. The next refinement is therefore an explicit
forward sonar-dome region, transition region, and main-hull region with shared
boundary constraints. Merely hiding the mismatch with a denser sampled surface
would not improve the parameterization.
