...
def _section_at(mesh: np.ndarray, x_target: float) -> np.ndarray | None:
    ...
    if mesh is None:
        return None
    # Axis 1 of the cached meshes runs longitudinally; axis 0 runs the girth.
    mesh = np.transpose(mesh, (1, 0, 2))
...
def main() -> None:
    ...
    underwater_rows = np.transpose(underwater, (1, 0, 2))
    aft_edge = underwater_rows[np.argmax(underwater_rows[:, :, 0].mean(axis=1))]
    axis = axes[0]
    axis.plot(
    ...
    if freeboard is not None:
        freeboard_rows = np.transpose(freeboard, (1, 0, 2))
        deck_edge = freeboard_rows[np.argmax(freeboard_rows[:, :, 0].mean(axis=1))]
        axis.plot(
    ...
    # each height.
    # Sampling the reference builds CSDL variables, so it needs a recorder.
    sampling_recorder = csdl.Recorder(inline=True)
    sampling_recorder.start()
    ...
    def waterline_from_mesh(meshes, level, x_lo, x_hi):
        ...
        for mesh in meshes:
            if mesh is None:
                continue
            oriented = np.transpose(mesh, (1, 0, 2))
            rows.extend(oriented[i] for i in range(oriented.shape[0]))
